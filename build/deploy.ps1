...
aws lambda update-function-code --function-name wegmans-auth --s3-bucket wegmans-lambda-builds --s3-key build.zip --profile wegmans;
aws lambda update-function-configuration --function-name wegmans-auth --handler 'lambda/server/index.handler' --profile wegmans;
